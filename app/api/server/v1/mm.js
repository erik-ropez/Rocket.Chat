...
*/
API.v1.addRoute(['mm.create'], { authRequired: true }, {
	post() {
		...
		if (!users) {
			throw new Meteor.Error('error-room-not-found', 'The required "username" or "usernames" param provided does not match any mass message');
		}

		const room = Meteor.call('createMassMessage', ...users);
...
/*
API.v1.addRoute(['mm.close'], { authRequired: true }, {
	post() {
...
API.v1.addRoute(['mm.counters'], { authRequired: true }, {
	get() {
...
API.v1.addRoute(['mm.files'], { authRequired: true }, {
	get() {
		const findResult = findDirectMessageRoom(this.requestParams(), this.user);
...
API.v1.addRoute(['mm.history'], { authRequired: true }, {
	get() {
		const findResult = findDirectMessageRoom(this.requestParams(), this.user);
...
API.v1.addRoute(['mm.members'], { authRequired: true }, {
	get() {
		const findResult = findDirectMessageRoom(this.requestParams(), this.user);
...
API.v1.addRoute(['mm.messages'], { authRequired: true }, {
	get() {
		const findResult = findDirectMessageRoom(this.requestParams(), this.user);
...
API.v1.addRoute(['mm.messages.others'], { authRequired: true }, {
	get() {
...
API.v1.addRoute(['mm.list'], { authRequired: true }, {
	get() {
		const { offset, count } = this.getPaginationItems();
...
API.v1.addRoute(['mm.list.everyone'], { authRequired: true }, {
	get() {
		if (!hasPermission(this.userId, 'view-room-administration')) {
			return API.v1.unauthorized();
		}
...
API.v1.addRoute(['mm.open'], { authRequired: true }, {
	post() {
		const findResult = findDirectMessageRoom(this.requestParams(), this.user);
...
API.v1.addRoute(['mm.setTopic'], { authRequired: true }, {
	post() {
